...
class PatchMerging2D(nn.Module):
    r""" Patch Merging Layer. Just like PoolLayer.
    Args:
...
class VSSLayer(nn.Module):
    ...
    def forward(self, x):
        ...
        if self.downsample is not None:
            x = self.downsample(x)
            # print(f"VSSLayer: output_after_downsample.shape {x.shape}")
...
class VSSM(nn.Module):
    def __init__(self, patch_size=4, in_chans=3, num_classes=1000, depths=[2, 2, 4, 2], depths_decoder=[2, 9, 2, 2],
                 dims=[96,192,384,768], dims_decoder=[768, 384, 192, 96, 16], d_state=16, drop_rate=0., attn_drop_rate=0., drop_path_rate=0.1,
        ...
        # 20240803: FXW modified head as deeper MLP==========
        self.head = nn.ModuleList()
        for i in range(len(dims_decoder) - 1):
            if i == 0:
                self.head.append(nn.Linear(self.num_features, dims_decoder[i+1]))
            else:
                self.head.append(nn.Linear(dims_decoder[i], dims_decoder[i+1]))

        if num_classes > 0:
            self.head.append(nn.Linear(dims_decoder[-1], num_classes))
        else:
        ...
        self.positive_weight = nn.Parameter(torch.tensor(1.0))
        self.negative_weight = nn.Parameter(torch.tensor(1.0))
